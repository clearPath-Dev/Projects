...
class ContentModalManager {
  ...
  openContactForm() {
    ...
    // Get contact form data and populate modal
  ...
  setupContactForm(data) {
    ...
    // Clone the form block and make it visible
    const formClone = data.formBlock.cloneNode(true);
    formClone.style.setProperty('display', 'block', 'important');

    // Remove any existing IDs to prevent conflicts and assign new unique IDs
    const form = formClone.querySelector('#wf-form-Contact-Form');
    if (form) {
      const uniqueId = 'modal-contact-form-' + Date.now();
      form.id = uniqueId;

      // Update all form elements to remove webflow's processed state
      const inputs = form.querySelectorAll('input, textarea, select');
      inputs.forEach(input => {
        // Remove webflow's data attributes that prevent re-initialization
  ...
  bypassWebflowLinkHandling() {
    ...
    links.forEach(link => {['data-wf-page', 'data-w-tab', 'data-wf-link'].forEach(attr => {link.removeAttribute(attr);});
    link.addEventListener('click', function(e) {e.stopImmediatePropagation(); }, true); });}
  ...
  addHoneypotValidation() {
    ...
    // Manual validation since Webflow isn't handling it
      ...
      requiredFields.forEach(field => {
        if (!field.value.trim()) {
          isValid = false;
          field.style.borderColor = 'red'; // Visual feedback
          field.focus();
        } else {
    ...
    if (submitBtn) {
      // Create a proper submit button that triggers form events
      const newSubmitBtn = submitBtn.cloneNode(true);
      // Replace the old button
      submitBtn.parentNode.replaceChild(newSubmitBtn, submitBtn);
      // Add click handler that properly submits the form
      newSubmitBtn.addEventListener('click', (e) => {
        e.preventDefault();
      

        // Force native validation to show
        if (!form.checkValidity()) {
          form.reportValidity();
          return;
        }

        // Create and dispatch a submit event that Webflow can intercept
        const submitEvent = new Event('submit', {
          bubbles: true,
          cancelable: true
        });

        // Let Webflow handle the submission (don't call form.submit())
...
// Performance optimization
  function optimizeWillChange() {document.querySelectorAll(".menu-block").forEach(block => {
...
// Main initialization
  window.addEventListener("DOMContentLoaded", () => {
    ...
    optimizeWillChange();
      ...
      el.addEventListener('click', (e) => {e.preventDefault();openContactForm();window.contentModalManager.handleUrlRouting();});});
  
  document.querySelectorAll('.privacy-trigger').forEach(el => {el.addEventListener('click', (e) => {e.preventDefault();
      openPrivacyPolicy();});});
  ...
  document.querySelectorAll(".menu-block").forEach(block => {
    ...
    const loop = gsap.to(marquee, { x: `-=${group.offsetWidth || 1}`, duration: 20, ease: "none", repeat: -1, 
      modifiers: { x: gsap.utils.unitize(x => parseFloat(x) % (group.offsetWidth || 1)) }, paused: true });
    block.onmouseenter = () => loop.play();block.onmouseleave = () => loop.pause();
    
    document.querySelectorAll('.contact-menu-trigger').forEach(trigger => {
      trigger.addEventListener('click', (e) => {e.preventDefault();e.stopPropagation();window.openContactForm();});});});
...
// Scroll and distribution system (condensed)
function initializeScrollAndDistribution() {
  ...
  // Distribute CMS items and setup image hovers
...
if (config.sectionClass === 'project-section') {
  items.forEach((item, index) => {
    ...
    cols[columnIndex].appendChild(item);});}else {
      // Keep existing distribution for vertical sections
      items.forEach((item, index) => {
        const columnIndex = index % cols.length;
...
setTimeout(() => { ScrollTrigger.refresh();
  document.querySelectorAll(".section-frame").forEach(sf => {
    ...
    ScrollTrigger.create({
      trigger: sf,start: "top 75%",end: "bottom 25%",toggleActions: "play none none reverse",
      onEnter: () => {
        ...
        titles.forEach(title => {setTimeout(() => window.scrambleManager?.triggerScramble(title), 500);});}});});
...
setTimeout(() => {ScrollTrigger.refresh();
  document.querySelectorAll(".section-frame .horizontal-scroll-container").forEach((container, i) => {
    try {
      ...
      wrapper.style.display = 'none';wrapper.offsetHeight;wrapper.style.display = '';
      ...
      if (scrollDist < 75) {scrollDist = wrapper.scrollWidth > container.offsetWidth ? wrapper.scrollWidth - container.offsetWidth + 75 : 75;}
      
      if (scrollDist > 0) {ScrollTrigger.create({trigger: sf,start: "top top",end: "bottom bottom",scrub: 1,animation: gsap.to(wrapper, { x: -scrollDist, ease: "none" }),onUpdate: (self) => {
            const newScrollLeft = self.progress * scrollDist;
            if (Math.abs(container.scrollLeft - newScrollLeft) > 5) {container.scrollLeft = newScrollLeft;}}});
         container.addEventListener('scroll', () => {if (!st.isActive) return;
          const progress = container.scrollLeft / scrollDist;st.progress(progress);});}
    } catch (error) {}});setTimeout(() => ScrollTrigger.refresh(), 500);}, 500);

      window.addEventListener('resize', () => {clearTimeout(horizontalScrollResizeTimeout);
      horizontalScrollResizeTimeout = setTimeout(() => {ScrollTrigger.refresh();}, 250);});});
  
    // Bio text system
  const bioSection = document.querySelector('.bio-section');{if (bioSection) {const triggers = bioSection.querySelectorAll('.bio-trigger');
    ...
    paragraphs.forEach((p, i) => {if (i === 0) {gsap.set(p, { opacity: 1, x: 0, display: 'block' });} else {gsap.set(p, { opacity: 0, x: 100, display: 'none' });}});
    ...
    let resizeTimer;
    window.addEventListener('resize', () => {clearTimeout(resizeTimer);resizeTimer = setTimeout(() => {const newIsMobile = window.innerWidth <= 768;
    if (newIsMobile !== isMobile) {setTimeout(() => { initializeBioTextSystem(); }, 100);}}, 250);});}}
        ...
        <div class="portfolio-modal">
          ...
          <div class="modal-left">
            ...
            <div class="modal-zoom-controls" id="zoomControls" style="display: none;">
              <button class="zoom-btn" id="zoomIn">+</button><button class="zoom-btn" id="zoomOut">-</button><button class="zoom-btn" id="zoomReset">⌂</button>
            </div>
            <div class="modal-image-thumbnails" id="imageThumbnails"></div>
          </div>
          <div class="modal-right"><h2 class="modal-title" id="modalTitle"></h2><div class="modal-description" id="modalDescription1"></div><div class="modal-description" id="modalDescription2"></div><div class="modal-links" id="modalLinks"></div></div>
        </div>
      </div>
    `);
  }
  ...
  let currentImages = [], currentImageIndex = 0, zoomLevel = 1, panX = 0, panY = 0, isPanning = true;
  ...
  const openModal = (data) => {
    ...
    currentImageIndex = 0;setupImages();setupLinks(data);modal.classList.add('active');document.body.style.overflow = 'hidden';};

  const closeModal = () => {modal.classList.remove('active');document.body.style.overflow = '';zoomLevel = 1; panX = panY = 0;mainImage.style.transform = 'scale(1) translate(0, 0)';};
  ...
  const setupImages = () => {
    if (!currentImages.length) {
      ...
      return;}
    const img = currentImages[currentImageIndex];mainImage.src = img.src;
    ...
    const thumbs = document.getElementById('imageThumbnails');if (currentImages.length > 1) {
      thumbs.innerHTML = currentImages.map((im, i) => `<img src="${im.src}" class="modal-thumbnail ${i === currentImageIndex ? 'active' : ''}" onclick="switchToImage(${i})">`).join('');
      thumbs.style.display = 'flex';} else thumbs.style.display = 'none';};

  const setupLinks = (data) => {const links = document.getElementById('modalLinks');links.innerHTML = '';
    ...
    ['github', 'figma', 'live'].forEach(type => {const url = data[`${type}Url`];if (url && url !== '' && url !== '#') {
        const a = document.createElement('a');a.href = url; a.target = '_blank';a.className = `modal-links ${type}Url`;
        const content = linkData[type]?.svg || linkData[type]?.text || type.toUpperCase();
        a.innerHTML = content;links.appendChild(a);}});
  document.getElementById('modalLinks').querySelectorAll('a[href]').forEach(l => l.addEventListener('click', e => e.stopImmediatePropagation(), true));};

  window.switchToImage = (index) => {if (index === currentImageIndex) return;currentImageIndex = index;
    gsap.to(mainImage, { opacity: 0, duration: 0.2, onComplete: () => { setupImages(); gsap.to(mainImage, { opacity: 1, duration: 0.3 }); } });};

  // Event listeners
  document.getElementById('closeModal').onclick = closeModal;
  modal.onclick = e => e.target === modal && closeModal();
  document.getElementById('zoomIn').onclick = () => { zoomLevel = Math.min(zoomLevel * 1.5, 4); mainImage.style.transform = `scale(${zoomLevel}) translate(${panX}px, ${panY}px)`; };
  document.getElementById('zoomOut').onclick = () => { zoomLevel = Math.max(zoomLevel / 1.5, 1); if (zoomLevel === 1) panX = panY = 0; mainImage.style.transform = `scale(${zoomLevel}) translate(${panX}px, ${panY}px)`; };
  document.getElementById('zoomReset').onclick = () => { zoomLevel = 1; panX = panY = 0; mainImage.style.transform = 'scale(1) translate(0, 0)'; };
	let startX = 0, startY = 0;
  mainImage.onmousedown = (e) => { if (zoomLevel <= 1) return; isPanning = true; startX = e.clientX - panX; startY = e.clientY - panY; mainImage.style.cursor = 'grabbing'; e.preventDefault();};
  document.onmousemove = (e) => {if (!isPanning) return;panX = e.clientX - startX;panY = e.clientY - startY;mainImage.style.transform = `scale(${zoomLevel}) translate(${panX}px, ${panY}px)`;};
  document.onmouseup = () => {isPanning = false;mainImage.style.cursor = zoomLevel > 1 ? 'grab' : 'default';};

  const getModalData = (section, index) => {const dataSection = document.querySelector('.modal-data-section');const container = dataSection?.querySelector(`[data-section="${section}"]`);
    const item = container?.querySelectorAll('.modal-item')[index];if (!item) return null;
const title = item.querySelector('.name')?.textContent?.trim() || 'Untitled Project';
    ...
    let images = [];const imageTypesText = item.querySelector('.image-types')?.textContent?.trim();
    const imageTypes = imageTypesText ? imageTypesText.split(',').map(t => t.trim()) : [];
    
     /*const systemsHover = item.querySelector('.systems-hover');if (systemsHover?.src && !systemsHover.src.includes('placeholder')) {
    images.push({ src: systemsHover.src, type: ['prototype', 'diagram', 'product'][parseInt(imageTypes[0] || '0')] || 'prototype' });} */
  item.querySelectorAll('.modal-images img, .modal-images .systems-hover').forEach((img, i) => {if (img.src && !img.src.includes('placeholder')) {const typeIndex = parseInt(imageTypes[i] || '2');
    images.push({ src: img.src, type: ['prototype', 'diagram', 'product'][typeIndex] || 'product' });}});
    const getUrl = sel => {const el = item.querySelector(sel); return el?.href || el?.textContent?.trim() || '';};
    return { title, description1, description2, images, githubUrl: getUrl('.github-url .w-inline-block'), figmaUrl: getUrl('.figma-url .w-inline-block'), liveUrl: getUrl('.live-url .w-inline-block') };};

['work', 'project', 'systems'].forEach(section => {const sec = document.getElementById(section);
  if (!sec) return;sec.querySelectorAll('.w-dyn-item').forEach((item, i) => {const wrapper = item.querySelector('.img-wrapper');
    if (wrapper && !wrapper.hasModalHandler) {wrapper.hasModalHandler = true;wrapper.style.cursor = 'pointer';
      wrapper.onclick = e => { e.preventDefault(); e.stopPropagation();const title = item.querySelector('.name')?.textContent?.trim();const modalItems = document.querySelector('.modal-data-section')?.querySelector(`[data-section="${section}"]`)?.querySelectorAll('.modal-item');
        let idx = i;if (modalItems && title) {modalItems.forEach((m, j) => { if (m.querySelector('.name')?.textContent?.trim() === title) idx = j; });}
        openModal(getModalData(section, idx) || { title: item.querySelector('.name')?.textContent || 'Untitled', description1: '<p>Project details coming soon.</p>', images: [] });  };}});});
  document.onkeydown = e => {if (!modal.classList.contains('active')) return;if (e.key === 'Escape') closeModal();
