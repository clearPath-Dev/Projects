Promise.all([
    ...
    class TextScramble {
        ...
        update() {
            ...
            for(let i = 0, n = this.queue.length; i < n; i++){
                let { from: from, to: to, start: start, end: end, char: char } = this.queue[i];
                if (this.frame >= end) {
                    complete++;
                    output += to;
    ...
    class ScrambleManager {
        ...
        triggerScramble(element) {
            ...
            if (data) data.scrambler.setText(data.originalText);
        }
        scrambleAll() {
    ...
    class SideMenuManager {
        ...
        checkViewportSize() {
            ...
            } else if (!this.isMobile && this.sectionMenus.size === 0) this.createSectionMenus();
            else if (this.isMobile && !this.burgerMenu) this.createBurgerMenu();
        }
        handleResize() {
            this.checkViewportSize();
        }
        ...
        createBurgerMenu() {
            ...
            this.burgerOverlay.querySelectorAll('.mobile-menu-item').forEach((item)=>{
                if (item.dataset.section === 'contact') item.onclick = ()=>{
        ...
        toggleBurgerMenu() {
            this.isBurgerOpen ? this.closeBurgerMenu() : this.openBurgerMenu();
        }
        openBurgerMenu() {
        ...
        createSideMenuHTML(currentSection) {
            ...
            otherSections.forEach((sectionName)=>{
                ...
                if (sectionName === 'contact') listItem.onclick = ()=>window.openContactForm();
                else listItem.onclick = ()=>this.navigateToSection(sectionName);
        ...
        setupIntersectionObserver() {
            const observer = new IntersectionObserver((entries)=>{
                entries.forEach((entry)=>{
                    ...
                    if (this.allSections.includes(sectionId)) entry.isIntersecting ? this.handleSectionEnter(sectionId) : this.handleSectionLeave(sectionId);
                });
            }, {
                threshold: 0.5
            });
        ...
        setupLoadObservers() {
            this.allSections.forEach((name)=>this.observeSectionContent(name));
        }
        observeSectionContent(sectionName) {
            const el = document.getElementById(sectionName);
            if (!el) return;
            new MutationObserver((mutations)=>{
                if (mutations.some((m)=>m.type === 'childList' && m.addedNodes.length > 0)) setTimeout(()=>this.markSectionAsLoaded(sectionName), 300);
            }).observe(el, {
                childList: true,
                subtree: true
            });
        ...
        observeImageLoading(sectionName) {
            ...
            images.forEach((img)=>{
                ...
                else img.onload = img.onerror = ()=>++loaded >= images.length && this.markSectionAsLoaded(sectionName);
            });
        ...
        handleSectionEnter(sectionName) {
            ...
            if (this.currentSection) this.isMobile ? this.updateBurgerMenuItems() : this.hideSideMenu(this.currentSection);
            this.currentSection = sectionName;
            if (this.isMobile) this.updateBurgerMenuItems();
            else if (this.sectionLoadStates.get(sectionName)) setTimeout(()=>this.showSideMenu(sectionName), 500);
        }
        ...
        showSideMenu(sectionName) {
            ...
            gsap.fromTo(items, {
                ...
                onComplete: ()=>items.forEach((item)=>{
                        item.onmouseenter = ()=>gsap.to(item, {
                                x: -60,
                                duration: 0.4,
                                ease: "power2.out"
                            });
                        item.onmouseleave = ()=>gsap.to(item, {
                                x: 0,
                                duration: 0.3,
                                ease: "power2.in"
                            });
    ...
    `);
        ...
        setupEventListeners() {
            ...
            window.addEventListener('popstate', ()=>{
                ...
                else this.handleUrlRouting();
            });
        }
        openModal(options = {}) {
            const { title: title = 'Modal Title', content: content = '<p>No content provided</p>', type: type = 'default', width: width = 'medium' } = options;
        ...
        handleUrlRouting() {
            ...
            if (path === '/privacy-policy') this.openPrivacyPolicy();
            else if (path === '/contact') this.openContactForm();
        }
        updateUrl(path) {
                ...
                form.addEventListener('submit', (e)=>{
                    ...
                    requiredFields.forEach((field)=>{
                        if (!field.value.trim()) {
                            ...
                            field.focus();
                        } else field.style.borderColor = ''; // Reset
                    });
                    if (!isValid) {
                        e.preventDefault();
                        return false;
    ...
    // Initialize systems
    function initializeEnhancedSideMenu() {
        window.sideMenuManager = new SideMenuManager();
    }
    // Global functions
    ...
    window.addEventListener("DOMContentLoaded", ()=>{
        ...
        document.querySelectorAll(".menu-block").forEach((block)=>{
            ...
            const loop = gsap.to(marquee, {
                ...
                repeat: -1,
                modifiers: {
                    x: gsap.utils.unitize((x)=>parseFloat(x) % (group.offsetWidth || 1))
                },
                paused: true
            });
        ...
        typeof Webflow !== 'undefined' && Webflow.push ? Webflow.push(initializeScrollAndDistribution) : setInterval(()=>typeof Webflow !== 'undefined' && Webflow.push && (clearInterval(module.exports), Webflow.push(initializeScrollAndDistribution)), 100);
    });
    ...
    function initializeScrollAndDistribution() {
        const configs = [
            ...
            {
                sectionClass: "systems-section",
                listClass: "systems-list",
                columnClasses: [
                    "systems-col-1",
                    "systems-col-2",
                    "systems-col-3"
        ...
        configs.forEach((config)=>{
            ...
            if (config.sectionClass === 'project-section') items.forEach((item, index)=>{
                const columnIndex = index % cols.length;
                const randomOffset = Math.floor(Math.random() * 100);
                const existingTransform = getComputedStyle(item).transform;
                const newTransform = existingTransform === 'none' ? `translateY(${randomOffset}px)` : `${existingTransform} translateY(${randomOffset}px)`;
                item.style.transform = newTransform;
                cols[columnIndex].appendChild(item);
            });
            else // Keep existing distribution for vertical sections
        ...
        if (bioSection) {
            ...
            function switchParagraph(newIndex) {
                ...
                tl.to(currentParagraph, {
                    ...
                    duration: 0.4,
                    ease: "power2.in",
                    onComplete: ()=>gsap.set(currentParagraph, {
                            display: 'none'
                        })
                });
                tl.set(newParagraph, {
                ...
                if (textContainer) tl.to(textContainer, {
                    scrollTop: 0,
                    duration: 0.3,
                    ease: "power2.out"
                }, "-=0.3");
            ...
            window.addEventListener('resize', ()=>{
                ...
                resizeTimer = setTimeout(()=>{
                    ...
                    if (newIsMobile !== isMobile) setTimeout(()=>{
                        initializeBioTextSystem();
                    }, 100);
                }, 250);
            });
        }
        // Portfolio modal system
        if (!document.getElementById('portfolioModal')) document.body.insertAdjacentHTML('beforeend', `
        ...
        <div class="portfolio-modal">
          ...
          <div class="modal-left">
            ...
            <div class="modal-zoom-controls" id="zoomControls" style="display: none;">
              <button class="zoom-btn" id="zoomIn">+</button><button class="zoom-btn" id="zoomOut">-</button><button class="zoom-btn" id="zoomReset">\u{2302}</button>
            </div>
    ...
    `);
        ...
        const openModal = (data)=>{
            ...
            currentImages = data.images?.length ? data.images : data.imageUrl ? [
                {
                    src: data.imageUrl,
                    type: data.imageType || 'product'
                }
            ] : [];
            currentImageIndex = 0;
            setupImages();
        ...
        const setupLinks = (data)=>{
            ...
            const linkData = {
                github: {
                    svg: `<svg class="icon" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>`
                },
                figma: {
                    svg: `<svg class="icon" viewBox="0 0 24 24"><path d="M12 8V0h4a4 4 0 1 1 0 8h-4zm0 0v8h4a4 4 0 0 0 0-8h-4zm0 8v4a4 4 0 1 1-8 0 4 4 0 0 1 8 0zm0-8H8a4 4 0 1 1 0-8h4v8zm0 0H8a4 4 0 0 0 0 8h4V8z"/></svg>`
                },
                live: {
                    text: "Visit Site \u2192"
                }
    ...
    images.push({ src: systemsHover.src, type: ['prototype', 'diagram', 'product'][parseInt(imageTypes[0] || '0')] || 'prototype' });} */ item.querySelectorAll('.modal-images img, .modal-images .systems-hover').forEach((img, i)=>{
            ...
            sec.querySelectorAll('.w-dyn-item').forEach((item, i)=>{
                ...
                if (wrapper && !wrapper.hasModalHandler) {
                    ...
                    wrapper.onclick = (e)=>{
                        ...
                        let idx = i;
                        if (modalItems && title) modalItems.forEach((m, j)=>{
